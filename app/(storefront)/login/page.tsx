...
function CustomerLoginForm() {
  ...
  const { customer, login, register } = useCustomer();
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-6 sm:p-10 rounded-3xl bg-white border border-[#eae7e2] shadow-xl text-[#141414] font-sans relative overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="block text-[11px] font-bold text-[#525252] uppercase tracking-wider">
              Password *
            </label>
            {tab === 'login' && (
              <span className="text-[10px] text-[#8c827a]">
                Standard Maison Access
              </span>
            )}
          </div>
          <div className="relative">
      ...
      {/* Footer Return Link */}
      <div className="mt-8 pt-6 border-t border-[#eae7e2] flex items-center justify-between text-xs">
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 font-semibold text-[#6b6b6b] hover:text-[#b87414] transition"
        >
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-[#8c827a]">
          ...
          <span>Encrypted Patron Session</span>
        </div>
      </div>
...
export default function CustomerLoginPage() {
  return (
    <div className="min-h-[85vh] bg-[#faf8f5] flex items-center justify-center px-4 py-16">
      <Suspense fallback={
        <div className="text-center text-neutral-500 flex items-center gap-2">
          <Loader2 className="w-5 h-5 animate-spin text-amber-500" />
          <span className="text-xs">Connecting to Atelier...</span>
        </div>
      }>
        <CustomerLoginForm />
      </Suspense>
